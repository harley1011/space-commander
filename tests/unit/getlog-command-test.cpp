...
TEST(GetLogTestGroup, Execute_OPT_DATE_OPT_SUB_getTgz_returnsCorrectFile)
{
    ...
    CHECK(*(result + 2 + GETLOG_INFO_SIZE + UTEST_SIZE_OF_TEST_FILES + 1) == EOF);
...
TEST(GetLogTestGroup, Execute_OPT_NOOPT_get2TGZ_returns2OldestTgz)
{
    ...
    if (pFile) {
        fwrite(result + 2 + GETLOG_INFO_SIZE, 1, 6, pFile);        // TODO fix this to read until EOF, or add the size to the result buffer
        fclose(pFile);
    }
    ...
    if (pFile) {
        fwrite(result + 2 + 6 + 2 * GETLOG_INFO_SIZE + GETLOG_ENDBYTES_SIZE, 1, 6, pFile); // TODO fix this to read until EOF, or add the size to the result buffer
        fclose(pFile);
    }

    CHECK(*(result + GETLOG_INFO_SIZE + 2 + 6) == EOF);
    CHECK(*(result + GETLOG_INFO_SIZE + 2 + 7) == EOF);
    CHECK(diff(dest, path));     
